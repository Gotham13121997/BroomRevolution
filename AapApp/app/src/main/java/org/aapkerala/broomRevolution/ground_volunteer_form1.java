...
public class ground_volunteer_form1 extends AppCompatActivity {
    //first page of ground volunteer form
    ...
    //for converting string into resource
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //taking the phone no and also wether offline
        if (extras != null) {
            phid=extras.getLong("phid");
            offline=extras.getBoolean("offline");
        }
        //respective variables for respective fields
        ...
        //hiding the delete button and expanding submit button to fit the screen
        ...
        pc.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                pan_mu.setText("");
                if(!sel.equals("")) {
                    //for all arrays except the following they have been named as name+"1"
                    //as there are pc and lac names common
                    ...
                    //removing the "All" option from string array
                    //and making array adapter out of it
        ...
        la.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                //some of the entries have -(no) in the end removing them
                if(sel.charAt(ind1+1)=='(')
                    sel=sel.substring(ind+2,ind1);
                //converting the middle space with _ so it can be matched
                else if(ind1!=ind+1)
                {
                ...
                {
                    //renaming chalakudy as chalakudy2 as there are arrays with same name
                    sel=sel.substring(ind+2);
                ...
                //removing the "All" entry from array and making an adapter out of it
                String[] cRaces = getResources().getStringArray(getStringIdentifier(ground_volunteer_form1.this, sel));
                String[] r=new String[cRaces.length-1];
                System.arraycopy(cRaces,1,r,0,cRaces.length-1);
        ...
        if (phid != 0L) {
            ...
            {
                //if getting data in in offline mode
                //getting data from sqlite database instead of firebase database
